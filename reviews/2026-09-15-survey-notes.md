...
Applied: the care sentence recast to the conduct form; the test 9 amendment drafted as a proposal (constitutional section, so a proposal is required); the case study's §12 extended. **Decided by Ben, 17 September:** the §4 proposal adopted with the conduct-form sentence and applied to §4 and the module; the Anchor 2 proposal adopted in the fourth round's revised form (the council's direction over his own) and applied; the test 9 proposal endorsed, its own round still to run before §9 changes; the council-variance experiment to the backlog (`README.md`, "Backlog") for a separate session with a literature review of existing instruments first; every outstanding raw filed, including the 4 and 10 September rounds' raws that had waited since those dates. Two additions directed: `../proposals/experience-welfare-standing-split.md`, and the insanity parallel's second edge recorded in the case study's §12.
